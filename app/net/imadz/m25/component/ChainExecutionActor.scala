...
import net.imadz.m25.business.ChainDsl
...
object ChainExecutionActor {
   ...
   * @param definition  编译好的 ChainDefinition（含 pipeline + reconfirm + router）
   * @param itemLoader  从 batchId 重新加载 items 的函数（恢复时使用）
   */
  def apply[Item](
      chainId: String,
      definition: ChainDsl.ChainDefinition[Item],
    ...
    Behaviors.setup { actorContext =>
      ...
      EventSourcedBehavior[Command, Event, State](
        ...
        emptyState = Idle,
        commandHandler = commandHandler(chainId, definition, processor, itemLoader, actorContext),
  ...
  def init[Item](
    ...
    sharding.init(
      Entity(EntityKey) { entityContext =>
        apply(chainId, definition, itemLoader)
      }
    )
  }
}
