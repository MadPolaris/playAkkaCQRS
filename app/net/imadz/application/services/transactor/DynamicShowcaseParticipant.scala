...
class DynamicShowcaseParticipant(val participantId: String) extends SagaParticipant[iMadzError, String, AppSagaContext] {
  import DynamicShowcaseParticipant._

  override protected val logger = LoggerFactory.getLogger(getClass)

  override protected def doPrepare(transactionId: String, context: AppSagaContext, traceId: String): ParticipantEffect[iMadzError, String] = 
    execute(transactionId, "prepare", traceId)

  override protected def doCommit(transactionId: String, context: AppSagaContext, traceId: String): ParticipantEffect[iMadzError, String] = 
    execute(transactionId, "commit", traceId)

  override protected def doCompensate(transactionId: String, context: AppSagaContext, traceId: String): ParticipantEffect[iMadzError, String] = 
    execute(transactionId, "compensate", traceId)

  private def execute(transactionId: String, phase: String, traceId: String): ParticipantEffect[iMadzError, String] = {
