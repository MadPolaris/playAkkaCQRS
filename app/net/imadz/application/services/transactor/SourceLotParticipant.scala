...
case class SourceLotParticipant(sourceLotId: Id, waferIds: Set[Id], waferNames: Set[String] = Set.empty)(implicit ec: ExecutionContext)
  ...
  override def doPrepare(transactionId: String, context: AppSagaContext, traceId: String): ParticipantEffect[iMadzError, String] = {
    val lotRef = context.lots.findLotById(sourceLotId)
    lotRef.ask(ReserveWaferRemoval(Id.of(transactionId), waferIds, waferNames, _))
  ...
  override protected def customClassification: PartialFunction[Throwable, SagaParticipant.RetryableOrNotException] = {
    case e: iMadzError =>
      logger.warn(s"[LotInvariant] source-lot reject: ${e.code}: ${e.message}")
