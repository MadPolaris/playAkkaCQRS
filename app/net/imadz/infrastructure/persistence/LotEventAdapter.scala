...
class LotEventAdapter extends EventAdapter[LotEvent, LotEventPO.Event] with LotProtoConverters {
  ...
  override def toJournal(e: LotEvent): LotEventPO.Event = e match {
    ...
    case evt: SubLotCreated => LotEventPO.Event.SubLotCreated(SubLotCreatedConv.toProto(evt))
    case evt: SubLotMerged => LotEventPO.Event.SubLotMerged(SubLotMergedConv.toProto(evt))
    case evt: SubLotScrapped => LotEventPO.Event.SubLotScrapped(SubLotScrappedConv.toProto(evt))
  ...
  override def fromJournal(p: LotEventPO.Event, manifest: String): EventSeq[LotEvent] = p match {
    ...
    case LotEventPO.Event.SubLotCreated(po) => EventSeq.single(SubLotCreatedConv.fromProto(po))
    case LotEventPO.Event.SubLotMerged(po) => EventSeq.single(SubLotMergedConv.fromProto(po))
    case LotEventPO.Event.SubLotScrapped(po) => EventSeq.single(SubLotScrappedConv.fromProto(po))
