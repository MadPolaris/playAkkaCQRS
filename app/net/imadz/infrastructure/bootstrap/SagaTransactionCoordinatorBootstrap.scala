...
trait SagaTransactionCoordinatorBootstrap extends ForSaga {
  ...
  def initSagaTransactionCoordinatorAggregate(sharding: ClusterSharding): Unit = {
  ...
  private def apply(transactionId: Id, tag: String): Behavior[SagaTransactionCoordinator.Command] = {
  ...
  private def createStepExecutor(context: ActorContext[SagaTransactionCoordinator.Command], key: String): ActorRef[StepExecutor.Command] = {
