...
// 输入：当前状态，(发起人, 接收人, 金额)
// 输出：Either[错误, 事件列表]
object InitiateTransactionRule extends InvariantRule[MoneyTransferTransactionEvent, MoneyTransferTransactionState, (String, String, Money)] {
  ...
  override def apply(state: MoneyTransferTransactionState, param: (String, String, Money)): Either[iMadzError, List[MoneyTransferTransactionEvent]] = {
    ...
    // 3. 决策：生成事件
      ...
      // 这里我们在 Rule 中生成"意图"，具体 ID 绑定留给 applyEvent 或由 Rule 参数传入 ID 会更合理。
      // 简化起见，我们假设参数里没传 ID，Rule 只决定"发生了这件事"。
