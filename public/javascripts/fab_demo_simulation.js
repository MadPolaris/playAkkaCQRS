/**
 * Fab Demo — Simulation Page JS
 *
 * UI control state + DOM rendering functions for the factory floor simulation.
...
(function() {
  ...
  /** Start a demo from a RouteDefinition (link from Route Designer page) */
  window.startRouteFromDesigner = function(routeId) {
    state.paused = false;
    window._demoPaused = false;
    _resetAllUI();
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // If navigated from Route Designer with ?route= param, auto-start the route
    var params = new URLSearchParams(window.location.search);
    var routeId = params.get('route');
    if (routeId) {
      // Clean the URL (remove query param) so refresh doesn't re-trigger
      window.history.replaceState({}, '', '/fab-demo');
      // Small delay to ensure observables are ready
      setTimeout(function() {
        window.startRouteFromDesigner(routeId);
      }, 300);
    }
  });
