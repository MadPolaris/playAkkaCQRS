...
本引擎采用 **向后回滚 (Backward Recovery)** 策略，确保分布式参与者之间的最终一致性。

#### 1. 事务阶段
- **Prepare 阶段**: 协调器按照定义的步骤组 (Step Group) 依次执行参与者的 `doPrepare` 逻辑。
- **Commit 阶段**: 仅当 **所有** Prepare 步骤都成功时触发，执行参与者的 `doCommit` 逻辑。
- **Compensate 阶段**: 当 Prepare 或 Commit 阶段中的 **任何** 步骤失败时触发，执行参与者的 `doCompensate` 逻辑。

#### 2. 故障恢复策略 (决策矩阵)

| 故障阶段 | 触发条件 | 引擎动作 | 最终状态预期 |
| :--- | :--- | :--- | :--- |
| **Prepare** | 业务逻辑错误 / 超时 | **执行 Compensate** | `FAILED` (一致性回滚) |
| **Commit** | 物理故障 / 超时 | **执行 Compensate** | `FAILED` (一致性回滚) |
| **Compensate** | 任何错误 / 超时 | **停止并挂起** | `SUSPENDED` (需人工修复) |

#### 3. 核心设计原则
- **选择性补偿**: 仅对在 Prepare 阶段 **已成功完成** 的参与者执行补偿。未开始或已失败的步骤在回滚时会被自动跳过。
- **结果幂等与覆盖 (Map 模式)**: 步骤结果记录在 `Map` 中。人工修复或成功的重试结果会覆盖之前的失败记录，从而允许状态机正确推进。
- **挂起状态 (Suspended)**: 如果补偿过程失败，事务将进入“挂起”状态。运维人员可以通过 UI/API 使用 **手动修复 (Manual Fix)** 功能修正参与者状态，然后触发 **重试阶段 (Retry Phase)** 完成最终回滚。
- **持久化保证**: 每一个状态转移都由 Akka Persistence (Event Sourcing) 提供支持。如果协调器宕机，它能恢复到精确的进度，包括已成功准备的步骤清单。
